...
```{r TRA -piechart, echo=FALSE, fig.align="center", fig.cap="", out.width="50%"}
...
```
The cumulative variance shows, that 72% of the total variance or in other means of the total information is explained by the first 2 principal components (PCs). These two PCs are sufficient for the analysis of the distribution of the breast cancer samples across the loadings of the KLK gene expression.  
...
The lung cancer microarray GSE149507 (Cai et al. 2021) were taken out of 6 patients with small cell lung cancer (SCLC). In each patient, the healthy lung tissue samples is adjacent to the carcinoma tissue and the data set thereby consists of a total of 12 samples.  
...
```
Most of the KLK gene expression boxplots seem to be lower than the overall median gene expression and thereby are clearly down-regulated. Interestingly, KLK4.4 clearly stands out agains as the highest expressed KLK gene. However, it needs to be considered that the expression of KLK4.4 is around the value 8, which does not really correspond with an over-expression in the context of the whole genome. Another interesting observation is that the boxplots of the KLK11 and KLK12 family are wide spread. With the whiskers of the boxplots, the gene expression spreads from a value of around 6 to about 9.5. This correlates with a higher variety in gene expression for these gene transcripts. KLK11 and KLK12 gene expression therefore has a high information value that should be recognizable in the following methods. This could also be due to the fact that the lung cancer data set consists of of both normal and healthy tissue, as in comparison to the breast cancer data set. Here, KLK11 and KLK12 are clear subject for further investigation in whether they are differently expressed between normal and carcinoma samples.
...
```
Notably, the samples are clustered according to their tissue type being lung carcinoma or healthy tissue. As you can see in the dendrogram at the top, the normal samples are distinctly clustered into one group with additionaly two more cancer samples. The other four cancer samples all form their own distinct group. This distribution by sample type clearly reflects itself in the KLK11 and KLK12 gene expression. While KLK4.4 is higher expressed for both normal and carcinoma samples, KLK11 and KLK12 isoforms are mainly higher expressed for the carcinoma sample. The only exception for this are the already mentioned carcinoma samples SCLC_01 and SCLC_03. Apart from this, the up-regulated gene expression for KLK11 and KLK12 are accompanied by the sample deriving from carcinoma tissue. 
(MAYBE NOCH NE REFERENZ EINBAUEN)
...
```
The cumulative variance shows, that 84% of the total variance or of the total information is explained by the first 2 principal components (PCs). These two PCs are sufficient for the analysis.  
The normal samples are definitely separated from the carcinoma samples. Considering the loadings, four of the cancer samples are characterized by KLK12 expression, while the two tumor samples SCLC_01 and SCLC_03 are mainly represented by KLK4.4 and KLK6 expression. Thereby, the PCA validates the observation of the heatmap.
