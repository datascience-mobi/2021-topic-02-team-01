...
#code was working with t-tests
combinations <- combn(ncol(all.KLK.rawdata.clean.),2)
...
```{r Test fails}
#Wilcoxon test 
#Welch two sampletest for every possible combination

combinations <- combn(ncol(all.KLK.rawdata.clean),2)
w.data <- adply(combinations, 2, function(x) {
  test <- wilcox.test(all.KLK.rawdata.clean[ ,x[1]],
                 all.KLK.rawdata.clean[ ,x[2]],
                 alternative = "two.sided") 
...
w.data <- apply(combinations, 2, function(x) {
  wilcox.test(
    all.KLK.rawdata.clean[,x[1]],
                 all.KLK.rawdata.clean[ ,x[2]],
                 alternative = "two.sided",
                 exact = FALSE) })
...
combinations <- combn(ncol(all.KLK.rawdata.clean),2)
apply(all.KLK.rawdata.clean, 2, function(x)
  lapply(combinations, function (y)
    wilcox.test(x[all.KLK.rawdata.clean %in% y],
             all.KLK.rawdata.clean[all.KLK.rawdata.clean %in% y])
    ))

#diffrent start: function between one gene and all others
w.test.gene.df <- function(gene, df){
  apply(df, 2, function(x){
    wilcox.test(x, gene, alternative = "two.sided", exact = FALSE)})
...
w.test.gene.df(all.KLK.rawdata.clean$KLK1, all.KLK.rawdata.clean)


w.test.gene.df.2 <- function(gene, df){
  apply(df, 2, function(x){
    wilcox.test(gene, x, alternative = "two.sided", exact = FALSE)})
  
  }

test2 <- w.test.gene.df.2(all.KLK.rawdata.clean$KLK1, all.KLK.rawdata.clean)
class(test2)
grep("test$statistic", test2)

as.data.frame(test2, "Var1" = "KLK1",
    "Var2" = colnames(df),
    "W.value"= test$statistic,
    "df"= test$parameter
