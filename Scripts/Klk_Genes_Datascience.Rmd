...
combinations <- combn(nrow(all.KLK.rawdata.clean),2)
...
w.data <- adply(combinations, 2, function(x) {
  ...
  out <- rbind.fill(data.frame(
    if colnames(all.KLK.rawdata.clean[x[1]]) ==0 
    else "Var1" = colnames(all.KLK.rawdata.clean[x[1]]),
    if  colnames(all.KLK.rawdata.clean[x[2]]) ==0 
    "Var2" = colnames(all.KLK.rawdata.clean[x[2]]),
                    "W.value"= test$statistic,
                    "df"= test$parameter,
                    "p.value" = test$p.value
...
#iteration über eine Variable, Variable hat keinen warnErrList(
#if n row ungleich 1 dann kann nicht mehr matchen... in welcher Situation kommt das vor?
#Python debugger - Punkt setzen in Code - Welcher Zustand haben welche Variablen 
# -> Wo ist der Fehler 
...
w.test.gene.df <- function(gene, df){
  apply(df, 1, function(x){
    wilcox.test(unlist(x), unlist(gene), alternative = "two.sided", exact = FALSE, paired = FALSE)})
  
  
}


test <- w.test.gene.df(all.KLK.rawdata.clean["KLK1", ], all.KLK.rawdata.clean)
test
class(test)
```
