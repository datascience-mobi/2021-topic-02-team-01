...
TRA.KLK.rawdata <- data.frame(t(TRA.KLK.rawdata))
...
#we got 18 identical isoform KLK TRAs!!! e.g. KLK1 and KLK1.1
...
#check for identical isoforms again 
...
length(which(cor.TRA.KLK.lung == 1)) #8 identical isoforms 

#removing identical isoforms
...
dev.off()
```
If we have have a look onto the 3 generated Heatmaps of 

```{r}
# We can even build a comparison with additional plots to non-TRA 
```
