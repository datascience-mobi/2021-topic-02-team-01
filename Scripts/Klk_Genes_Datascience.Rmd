...
t.data
#Wilcoxon test 
#Welch two sample test for every possible combination

combinations <- combn(ncol(all.KLK.rawdata.clean),2)
as.numeric(combinations)
combinations
...
w.test.gene.df <- function(gene, df){
  ...
  test <- wilcox.test(unlist(x), unlist(gene), 
                      alternative = "two.sided", 
                      exact = FALSE, 
...
KLK1.vs.all <- w.test.gene.df(all.KLK.rawdata.clean["KLK1", ], all.KLK.rawdata.clean)
KLK1.vs.all
...
significant.pvalues <- function(genevsall) {
...
#test run 
KLK1.vs.all.sig <- significant.pvalues(KLK1.vs.all) 
KLK1.vs.all.sig

#simple barplot 
barplot(KLK1.vs.all.sig)

#the test with KLK1 show that the gene is not significantly different expressed compared to all other genes of the dataset.
```
