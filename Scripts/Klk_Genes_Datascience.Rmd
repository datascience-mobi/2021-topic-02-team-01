...
Union.TRA[nrow(Klk.TRA1)+nrow(Klk.TRA2)+nrow(Klk.TRA3)+nrow(Klk.TRA4)+nrow(Klk.TRA5)+1:nrow(Klk.TRA6),5] <- " tra.mouse.gtex"


## Da Frau Dinkelacker eine Übersicht zu der Verteilung der KLKs auf den Chromosomen und die Filterung nach max.tissue haben möchte, hier kann jetzt bei Union.TRA beliebig nach allem gesucht werden übersichts-plots erstellt werden. Nur vorsicht das Mausdatensätze und Menschen in einem sind.
# Kann für die folgenden Analysen dafür genutzt werden um nach Datensatz
...
# unique KLK names
all.KLK.gene.symbols <- unique(Union.TRA$gene.symbol)
all.KLK.gene.symbols

# Now show overview of KLK distribution
# Max.tissue for tra.human
length.pancreas <- length(which(Union.TRA[which(Union.TRA$dataset == "tra.human"), 4] == "Pancreas") == TRUE)
length.prostate <- length(which(Union.TRA[which(Union.TRA$dataset == "tra.human"), 4] == "Prostate") == TRUE)
barplot(c(length.pancreas,length.prostate) ,main ="Distribution of tissue types for human KLK TRAs", )
```
...
###Isolating TRAs from the rawdata breast dataset
```{r Isolating KLK TRAs rawdata breast + Heatmaps}
...
KLK.genes.TRA.human = TRA.symbol[ind.human] #ind.human (see chunk 21) filters for KLKs in tra.human
...
TRA.KLK.rawdata <- data.frame(TRA.KLK.rawdata)
...
#test function 
cor.TRA.KLK.rawdata <- cor.genes(TRA.KLK.rawdata)
length(which(cor.TRA.KLK.rawdata == 1))
#we got 10 identical TRAs!!! e.g. KLK1 and KLK1.1
#Identical transcripts can cause issues later on for statistical testing ect. 

#remove identical columns
TRA.KLK.rawdata.clean <- TRA.KLK.rawdata[!duplicated(unclass(TRA.KLK.rawdata))]
dim(TRA.KLK.rawdata)
dim(TRA.KLK.rawdata.clean)
#removed 12 identical columns
...
TRA.KLK.rawdata
#Heatmap of KLK TRAs expression with identicals
setwd(here("Plots/Plots_raw_breast"))
pdf("KLK_TRAs_Heatmap_rawdata_wid.pdf")
pheatmap(t(TRA.KLK.rawdata),
...
TRA.KLK.rawdata.clean = data.frame(t(TRA.KLK.rawdata.clean[order(colnames(TRA.KLK.rawdata.clean)), ]))
TRA.KLK.rawdata.clean
```
...
TRA.KLK.breast = data.frame(t(breastExprs[ind.KLK.tra.human,]))
dim(TRA.KLK.breast) ## 39 KLK genes in breast cancer rawdata 
TRA.KLK.breast
#number of identical columns
cor.TRA.KLK.breast <- cor.genes(TRA.KLK.breast)
length(which(cor.TRA.KLK.breast == 1))
#17 identical columns 
...
#remove identical columns
TRA.KLK.breast.clean <- genes.cleanup(TRA.KLK.breast)
...
pdf("KLK_TRAs_Heatmap_breast_wid.pdf")
pheatmap(t(TRA.KLK.breast),
...
TRA.KLK.breast.clean = TRA.KLK.breast.clean[order(substr(rownames(TRA.KLK.breast.clean), 19, 22))
  ,order(colnames(TRA.KLK.breast.clean))]
TRA.KLK.breast.clean
...
setwd(here("Plots/Plots_breast"))
pdf("KLK_TRAs_Heatmap_breast_noclust.pdf")
pheatmap(t(TRA.KLK.breast.clean),
         main = "Expression values of KLK TRAs GSE65216 (Maire et al. 2013)",
         cellwidth = 15,
         cellheight = 8,
         ...
         cluster_cols = FALSE)
dev.off()
#no difference to previous heatmap
...
#boxplots 
setwd(here("Plots/Plots_breast"))
pdf("KLK_TRAs_boxplot_all.pdf")
boxplot(TRA.KLK.breast.clean,col=rainbow(length(rownames(TRA.KLK.breast.clean))),main="Expression values of KLK TRAs GSE65216 (Maire et al. 2013)",cex.axis=0.8)
dev.off()
...
TRA.KLK.lung = t(TRA.KLK.lung)
dim(TRA.KLK.lung) ## 39 KLK genes in breast cancer rawdata 

#finding duplicates
cor.TRA.KLK.lung <- cor.genes(TRA.KLK.lung)
length(which(cor.TRA.KLK.lung == 1)) #18 identical columns 

#removing identical columns
TRA.KLK.lung <- data.frame(TRA.KLK.lung)
TRA.KLK.lung.clean <- genes.cleanup(TRA.KLK.lung)
length(which(cor.genes(TRA.KLK.lung.clean) == 1))

#heatmap with duplicates
setwd(here("Plots/Plots_lung"))
pdf("KLK_TRAs_Heatmap_lung_wid.pdf")
pheatmap(t(TRA.KLK.lung),
...
Analysis rawdata breast: Boxplot, Heatmap
```{r}
#Boxplot:
boxplot(KLK.rawdata.removed,col=rainbow(length(rownames(KLK.rawdata.removed))),main="Gene expression of KLK genes in human breast cancer",cex.axis=0.8, sub = "Identical genes removed")

pheatmap(all.KLK.rawdata, main = "Heatmap genes expression KLK genes in human breast cancer")

x1 = cor(all.KLK.rawdata)
head(x1)
pheatmap(x1, main = "Correlation between the KLK TRAs ") 
# corr between patients
x2 = cor(t(all.KLK.rawdata))

head(x2)
pheatmap(x2, main = "Correlation between the patients of breast cancer")
```
...
class(klkexprs_lung)
dim(klkexprs_lung)
KLK.lung <- data.frame(t(klkexprs_lung))
KLK.lung

#calculate correlation
cor.KLK.lung <- cor(KLK.lung, method = "pearson")
round(cor.KLK.lung, 2)
cor.KLK.lung <- data.frame(cor.KLK.lung)
#delete diagonal and upper triangle
cor.KLK.lung[upper.tri(cor.KLK.lung)]= 0
diag(cor.KLK.lung)=0
cor.KLK.lung
which(cor.KLK.lung == 1)
length(which(cor.KLK.lung == 1))
#There are identical columns! corr=1

#Remove identical columns
KLK.lung.duplicated #! muss weg 
KLK.lung.removed <- KLK.lung[!duplicated(unclass(KLK.lung))]
dim(KLK.lung.removed)
KLK.lung.removed

#check for identical 
cor.KLK.lung.removed <- data.frame(cor(KLK.lung.removed))
diag(cor.KLK.lung.removed)=0
cor.KLK.lung.removed[upper.tri(cor.KLK.lung.removed)]= 0
length(which(cor.KLK.lung.removed == 1))
