...
loadings.test <- tl.breast.loadings %>% 
  group_by(name) %>%
  wilcox_test(value ~ mutation_type)
loadings.test
loadings.test <- loadings.test %>% add_xy_position(x="mutation_type")
loadings.test

setwd(here("Plots/Plots_breast"))
pdf("Wraped_boxplotloadings_breast.pdf")
ggplot(tl.breast.loadings, aes(x=name, y=value, fill = mutation_type))+
  ggtitle("PCA Loadings, GSE65216 (Maire et al. 2013) ")+
  geom_boxplot()+
  ...
  xlab("Genes") +
  ylab("Expression")+
  geom_segment(data = loadings.test)
  #stat_pvalue_manual(loadings.test, hide.ns = TRUE, tip.length = 0, step.increase = 0) 
    
  

  #geom_signif(data = annotation_df, aes(xmin=start, xmax=end), manual = TRUE)
  
  #stat_compare_means(aes(group = mutation_type), label = "p.format", paired = FALSE)
  #stat_pvalue_manual(loadings.test,  label = "p.adj.signif", tip.length = 0.01)
