...
### Visualization of gene expression - Boxplot
```{r Boxplot}
...
all.KLK.rawdata.clean <- t(all.KLK.rawdata.clean[order.rawdata]) #sorted all.KLK.breast.clean
...
abline(h = 0.92802 , col="blue", lty=5)
legend("topleft", legend=c("Cut-off @ PC4"),
       col=c("blue"), lty=5, cex=0.6)

# we can see in this cumulative variance plot, that we only need about 3 principle components to achieve a a total variance of %
...
plot(pr.all.KLK.rawdata.clean$x[,2],pr.all.KLK.rawdata.clean$x[,3],
     xlab="PC2 (13,88%)",
     ylab = "PC3 (10,26%)",
     main = "PC2 / PC3 - plot",
    col = color.raw)
...
## should we rename the chips by hand, so we can cluster for mutation?
...
### clean up of identical transcripts
```{r, cleanup identical transcripts}
...
dev.off()
```
```{r Overview and Histogram breast}
...
### Visualization of gene expression - Boxplots 
```{r}
# Use mixedsort function from the gtool package to sort the KLKs correctly
...
# run PCA on the KLK data frame
# center and scale = TRUE ? does vsnrma not already do this?
pr.all.KLK.breast.clean <- prcomp(all.KLK.breast.clean, center = TRUE, scale = TRUE)
#call summary
...
variance.raw <- cumsum(pr.all.KLK.breast.clean$sdev^2 / sum(pr.all.KLK.breast.clean$sdev^2))
plot(variance.raw[0:20], xlab = "PC #", ylab = "Amount of explained variance", main = "Cumulative variance plot")
abline(v = 4, col="blue", lty=5)
abline(h = 0.91110, col="blue", lty=5)
legend("topleft", legend=c("Cut-off @ PC4"),
       col=c("blue"), lty=5, cex=0.6)
...
color.raw = c(ifelse(annotation.hmap.breast[,2] == "TRA", "blue", "red"))
...
plot(pr.all.KLK.breast.clean$x[,2],pr.all.KLK.breast.clean$x[,3],
     xlab="PC2 (13,92%)",
     ylab = "PC3 (8,90%)",
     main = "PC2 / PC3 - plot",
    col = color.raw)
...
### cleanup of identical transcripts
...
# run PCA on the KLK data frame
# center and scale = TRUE ? does vsnrma not already do this?
pr.all.KLK.lung.clean <- prcomp(all.KLK.lung.clean, center = TRUE, scale = TRUE)
#call summary
...
variance.raw <- cumsum(pr.all.KLK.lung.clean$sdev^2 / sum(pr.all.KLK.lung.clean$sdev^2))
plot(variance.raw[0:12], xlab = "PC #", ylab = "Amount of explained variance", main = "Cumulative variance plot")
...
color.raw = c(ifelse(annotation.hmap.lung[,2] == "TRA", "blue", "red"))
...
plot(pr.all.KLK.lung.clean$x[,2],pr.all.KLK.lung.clean$x[,3],
     xlab="PC2 (13,00%)",
     ylab = "PC3 (4,83%)",
     main = "PC2 / PC3 - plot",
    col = color.raw)
