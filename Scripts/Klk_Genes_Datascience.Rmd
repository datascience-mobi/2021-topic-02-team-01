...
# variance explained by each of the 20 principle components
...
pr.all.KLK.lung.clean <- prcomp(t(all.KLK.lung.clean), center = TRUE, scale = FALSE)
#call summary
...
legend("left", legend=c("cancer", "normal"),
       col=c("blue", "red"), lwd= 10, cex=0.6)
...
#subset dataframe: only genes of cluster5
cluster5.lung.names <- c("KLK1.3","KLK12","KLK12.1", "KLK12.3","KLK12.4","KLK12.5","KLK12.6")
cluster5.lung.names <- as.matrix(cluster5.lung.names)
...
#Plot with p.values cluster 3 
setwd(here("Plots/Plots_lung"))
pdf("Cluster5_lung.pdf")
ggplot(tl.cluster5.lung, aes(x= name, y=value, fill= tissue_type))+
  ggtitle("Cluster 5, GSE149507 (Cai et al. 2021)")+
...
#filter the KLK10.3 from tl.lung df
cluster4.lung.names <- c("KLK8.8", "KLK10.3", "KLK11", "KLK11.2")
...
---#ggplot 
setwd(here("Plots/Plots_lung"))
pdf("Cluster4_lung.pdf")
ggplot(tl.cluster4.lung, aes(x= name, y=value, fill= tissue_type))+
...
```
```{r, Cluster 3}
b.cluster3.lung <- tl.all.KLK.lung.clean[, 1] %in% "KLK4.4"
tl.cluster3.lung <- which(b.cluster3.lung == TRUE)
tl.cluster3.lung <- as.data.frame(tl.all.KLK.lung.clean[tl.cluster3.lung, ])
tl.cluster3.lung

setwd(here("Plots/Plots_lung"))
pdf("Cluster3_lung.pdf")
ggplot(tl.cluster3.lung, aes(x= name, y=value, fill= tissue_type))+
  ggtitle("Cluster 3, GSE149507 (Cai et al. 2021)")+
  stat_boxplot(geom="errorbar")+
  geom_boxplot()+
  stat_compare_means(aes(group = tissue_type), label = "p.format", paired = TRUE, size = 3.5)
dev.off()
