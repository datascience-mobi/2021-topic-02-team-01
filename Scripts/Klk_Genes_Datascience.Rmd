...
pr.all.KLK.breast.clean <- prcomp(t(all.KLK.breast.clean), center = TRUE, scale = FALSE)
#call summary
...
color.breast.mutation = c(ifelse(samples.annotation.breast == "TNBC", yes = "#66c2a5", no = ifelse(samples.annotation.breast == "Her2", yes = "red", no = ifelse(samples.annotation.breast == "LumA", yes = "green", no = "yellow"))))
...
```{r}
# Perform the PCA
pca.breast <- prcomp(t(all.KLK.breast.clean), center = TRUE, scale = FALSE)
...
pca.breast.scores %>% 
  ...
  ggplot(aes(x = PC1, y = PC2)) +
    ggtitle("PCA GSE65216 (Maire et al. 2013)") +
    xlab("PC1 (46,49%)") +
    ylab("PC2 (25,39%)") +
  geom_point()

# Start with annotation and loadings
color.breast.mutation = c(ifelse(samples.annotation.breast == "TNBC", yes = "TNBC", no = ifelse(samples.annotation.breast == "Her2", yes = "red", no = ifelse(samples.annotation.breast == "LumA", yes = "green", no = "yellow"))))

mutations <- samples.annotation.breast[,1]

ggplot(pca.breast.scores, aes(x = PC1, y = PC2, color = mutations))+
      geom_point(size= 3)+
...
#Model 1 with all genes 
model1 <- glm(as.factor(tissue_type)~ ., family = binomial(), data = reg.lung.all)
summary(model1)
#error 61 not defined because of singularities??? -> Colinearity! 
...
model1 #why is the z value=0 and the p.value=1???? something is wrong!
