...
fviz_cluster(km.klk.rawdata, data=all.KLK.rawdata.clean, 
             palette =c("#2E9FDF","#00AFBB", "#FC4E07", "#E7B800", "#8DD3C7" ), 
             geom = "point", 
             ellipse.type = "convex",
             ggtheme = theme_bw(),
...
fviz_cluster(km.klk.lung.5, data=all.KLK.lung.clean.noGSM, 
             palette =c("#2E9FDF","#00AFBB", "#FC4E07", "#E7B800", "#009900"), 
             geom = c("point","text") ,
             ellipse.type = "convex",
             ggtheme = theme_bw(),
             labelsize = 6,
...
wssplot.klk.breast <- function(all.KLK.breast.clean.noGSM, nc=15, seed=1234)
{
  wss <- (nrow(all.KLK.breast.clean.noGSM)-1)*sum(apply(all.KLK.breast.clean.noGSM ,2,var))
  for(i in 2:nc){
    set.seed(seed)
    wss[i] <- sum(kmeans(all.KLK.breast.clean.noGSM , centers = i)$withinss)}
  plot(1:nc, wss, type="b", xlab="Number of Clusters", ylab="Within groups sum of squares")
}

wssplot.klk.breast(all.KLK.breast.clean.noGSM)

#approx 5 clusters

km.klk.breast <- kmeans(scale(all.KLK.breast.clean.noGSM),5, nstart=25)
km.klk.breast$cluster

fviz_cluster(km.klk.breast, data=all.KLK.breast.clean.noGSM, 
             palette =c("#009900","#00AFBB", "#FC4E07", "#E7B800", "#003333"), 
             geom = "point", 
             ellipse.type = "convex",
             ggtheme = theme_bw()
)
