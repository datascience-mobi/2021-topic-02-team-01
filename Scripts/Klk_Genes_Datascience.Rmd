...
pr.all.KLK.rawdata.clean <- prcomp(t(all.KLK.rawdata.clean), center = TRUE, scale = FALSE)
#call summary
...
# total weightage of each gene in this distribution
gene.load.raw <- apply(pr.all.KLK.rawdata.clean$rotation, 1, sum)

# arranging the weightage obtained above in a descending order by taking the absolute value of the total loadings for each gene
gene.differ.raw <- order(abs(gene.load.raw), decreasing = TRUE)

# show the top most different genes across the two groups
top.10.raw <- gene.differ.raw[1:10]

rownames(all.KLK.rawdata.clean[top.10.raw,])
...
# total weightage of each gene in this distribution
gene.load.breast <- apply(pr.all.KLK.breast.clean$rotation, 1, sum)
# arranging the weightage obtained above in a descending order by taking the absolute value of the total loadings for each gene
gene.differ.breast <- order(abs(gene.load.breast), decreasing = TRUE)

# show the top most different genes across the two groups
top.10.breast <- gene.differ.breast[1:10]

rownames(all.KLK.breast.clean[top.10.breast,])
...
# total weightage of each gene in this distribution
gene.load.lung <- apply(pr.all.KLK.lung.clean$rotation, 1, sum)
# arranging the weightage obtained above in a descending order by taking the absolute value of the total loadings for each gene
gene.differ.lung <- order(abs(gene.load.lung), decreasing = TRUE)

# show the top most different genes across the two groups
top.10.lung <- gene.differ.lung[1:10]

rownames(all.KLK.lung.clean[top.10.lung,]) #most different genes
```
...
cluster3.lung.names <- c("KLK1.3","KLK12","KLK12.1", "KLK12.3","KLK12.4","KLK12.5","KLK12.6")
cluster3.lung.names <- as.matrix(cluster3.lung.names)
b.cluster3.lung <- tl.all.KLK.lung.clean[, 1] %in% cluster3.lung.names
...
dev.off()
```
```{r, Underexpression KLK10.3}
#In the heatmap KLK10.3 seems underexpressed in the cancer tissue in comparison to the normal tissue 
#filter the KLK10.3 from tl.lung df
b.KLK10.3.lung <- tl.all.KLK.lung.clean[, 1] %in% "KLK10.3"
tl.KLK10.3.lung <- which(b.KLK10.3.lung == TRUE)
tl.KLK10.3.lung <- as.data.frame(tl.all.KLK.lung.clean[tl.KLK10.3.lung, ])
tl.KLK10.3.lung

#ggplot 
setwd(here("Plots/Plots_lung"))
pdf("KLK10.3.pdf")
ggplot(tl.KLK10.3.lung, aes(x= name, y=value, fill= tissue_type))+
  ggtitle("KLK10.3, GSE149507 (Cai et al. 2021)")+
  stat_boxplot(geom="errorbar")+
  geom_boxplot()+
  stat_compare_means(aes(group = tissue_type), label = "p.format", paired = TRUE, size = 3.5)
dev.off()
...
reg.tissue.type <- lm(tissue_type ~ KLK12+KLK10.3+KLK12+KLK12.3+KLK12.4+KLK12.5+KLK12.6, data = reg.lung.all)
