...
```{r Hypothesis testing - Data preperation}
...
#alternative less because non-cancerous tissue is first displayed -> H1: KLKxn is significantly lower expressed than KLKxca  --> if true, with an upper tail test the result is  KLKxca is significantly higher expressed than KLKxn

#test:
wilcox.test(as.numeric(all.KLK.lung.clean.n["KLK4.4", ]), as.numeric(all.KLK.lung.clean.ca["KLK4.4", ]), paired = TRUE, alternative = "less")

wilcox.test(as.numeric(all.KLK.lung.clean.ca["KLK4.4", ]), as.numeric(all.KLK.lung.clean.n["KLK4.4", ]), paired = TRUE, alternative = "greater")
#same p.value
...
titel <- "Identification of expression differences, GSE149507 (Cai et al. 2021)"



exprdiff <- ggarrange(gg.cluster1 + rremove("legend")+rremove("xlab"), 
                      gg.cluster4 + rremove("legend")+rremove("ylab")+rremove("xlab"),
                      ggPCA.ca + rremove("legend"),  
                      ggPCA.n+ rremove("legend")+rremove("ylab"), 
          labels = c("A", "B", "C", "D"), 
...
```
Identification of expression differences 
###Regression analysis
```{r, Regression analysis - data preperation}
...
#Model 0 :
#Rule of thumb: 1 variable per 10 ten events. (PAPER needed!)
...
modelKLK10.3.12 <- glm(tissue_type ~  KLK12+KLK10.3, family = binomial(), data = train.reg)
...
modelKLK10.3.7 <- glm(tissue_type ~  KLK10.3, family = binomial(), data = train.reg)
summary(modelKLK10.3.7)

#KLK10.3 with full dataset (10 mikrochips)
modelKLK10.3.10 <- glm(tissue_type ~  KLK10.3, family = binomial(), data = reg.lung.all)
summary(modelKLK10.3.10)
#KLK10.3 is not suited to predict the tissue type of a mikrochip


#KLK1.3
modelKLK1.3 <- glm(tissue_type ~  KLK1.3, family = binomial(), data = train.reg)
summary(modelKLK1.3)
plot(modelKLK1.3)
#p.value: 0.198

#KLK4.4
modelKLK4.4 <- glm(tissue_type ~  KLK4.4, family = binomial(), data = train.reg)
summary(modelKLK4.4)
...
#KLK12 model
modelKLK12 <- glm(tissue_type ~  KLK12, family = binomial(), data = train.reg)
summary(modelKLK12)
plot(modelKLK12)
...
#KLK1.3
summary(modelKLK1.3)
predictionKLK1.3 <- predict(modelKLK1.3, test.reg, type="response")

#KLK12 
summary(modelKLK12)
predictionKLK12 <- predict(modelKLK12, test.reg, type="response")


#results
predictionKLK1.3.classes <- ifelse(predictionKLK1.3 > 0.5, "ca", "n")
predictionKLK12.classes <- ifelse(predictionKLK12 > 0.5, "ca", "n")
predictionKLK1.3.classes
#
...
```{r}
#Trying to remove collinearity by removing genes with a correlation greater then 0.7
train.corr <- cor(train.reg)
train.corr[upper.tri(train.corr)]<- 0
diag(train.corr) <- 0
length(which(train.corr > 0.7))
train.corr.low <- train.reg[ , !apply(train.corr, 2, function(x){ any(x>0.7)})]
train.corr.low

train
modelKLK12.low <- glm(tissue_type ~  KLK12, family = binomial(), data = train.reg)
summary(model0)
plot(modelKLK12)

#with 0.7 still Colliniarity - set to 0.6
low.cor.reg.lung.all <- reg.lung.all[ , !apply(tmp, 2, function(x){ any(x>0.7)})]
