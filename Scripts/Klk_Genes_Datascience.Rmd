...
median(rawdataExprs)  #Median all genes: 6.627027
mean(rawdataExprs) #Mean all genes: 6.841523

median(unlist(all.KLK.rawdata.clean)) #Median all KLKs: 5.456886
mean(unlist(all.KLK.rawdata.clean))   #Mean all KLKs: 5.646926

median(unlist(TRA.KLK.rawdata.clean)) #Median TRA KLKs: 5.408611
mean(unlist(TRA.KLK.rawdata.clean))   #Mean TRA KLKs: 5.479232
###### The dataframe TRA.KLK.rawdata.clean is called here before it is even defined, we need to move the histograms chronological after heatmaps, if you want to call the command
###### median(unlist(TRA.KLK.rawdata.clean))
...
abline(v = 6.627027, col= "black", lwd = 2)#median all genes
text(2, 1, "Median all genes", pos=1)
abline(v = 5.456886, col= "blue", lwd = 2) #median all KLKs
abline(v = 5.408611, col= "red", lwd = 2) #median TRA KLKs
#Want to annotate what the lines are representing, but i cant 
dev.off()
```
...
all.KLK.rawdata.clean <- t(all.KLK.lung.clean[,order.rawdata]) #sorted all.KLK.breast.clean
...
abline(h=mean(unlist(all.KLK.rawdata.clean)), col= "gray", lwd = 1)
dev.off()
...
pr.all.KLK.rawdata.clean <- prcomp(all.KLK.rawdata.clean, center = TRUE, scale = TRUE)
#call summary
summary(pr.all.KLK.rawdata.clean)
# goal is to get information of roughly 95% of the KLK data -> 95% cumulative variance
# in the summary we can see that this is achieved by the 3rd pc
# the variance of the pcs rapidly declines, there is only abot 5% variance for the 3rd pc

# get graphical overview (not really needed though)

screeplot(pr.all.KLK.rawdata.clean, type = "l", npcs = 12, main = "Screeplot of the first 10 PCs")
abline(h = 1, col="red", lty=5)
legend("topright", legend=c("Eigenvalue = 1"),
       col=c("red"), lty=5, cex=0.6)

cumpro <- cumsum(pr.all.KLK.rawdata.clean$sdev^2 / sum(pr.all.KLK.rawdata.clean$sdev^2))
plot(cumpro[0:12], xlab = "PC #", ylab = "Amount of explained variance", main = "Cumulative variance plot")
abline(v = 3, col="blue", lty=5)
abline(h = 0.95094 , col="blue", lty=5)
...
# Plot the PCs
plot(pr.all.KLK.rawdata.clean$x[,1],pr.all.KLK.rawdata.clean$x[,2], xlab="PC1 (77.3%)", ylab = "PC2 (13%)", main = "PC1 / PC2 - plot")
plot(pr.all.KLK.rawdata.clean$x[,2],pr.all.KLK.rawdata.clean$x[,3], xlab="PC1 (13%)", ylab = "PC2 (4,8%)", main = "PC1 / PC2 - plot")
plot(pr.all.KLK.rawdata.clean$x[,1],pr.all.KLK.rawdata.clean$x[,3], xlab="PC1 (77.3%)", ylab = "PC2 (4,8%)", main = "PC1 / PC2 - plot")

# we see some sort of clustering
...
1. Data obtained by a random sampling method
2. Roughly normally distributed 
3. No extreme outliers 
```{r}
#Hypothesis testing with t-tests
#according to the histogram previosly created, KLks are not normally distributed
...
dev.off()
#not a normal distribution!!! -> Wilcoxon test 



#t-test KLK1
#H0 there is no significant difference between KLK1 and all other TRAs
#H1: KLK1 is higher expressed
#Welch two sample t-test for every possible combination

##the combinations are two sided t-tests
combinations <- combn(ncol(TRA.KLK.rawdata.clean),2)
w.data <- adply(combinations, 2, function(x) {
  test <- wilcox.test(TRA.KLK.rawdata.clean[ ,x[1]],
                 TRA.KLK.rawdata.clean[ ,x[2]],
                 alternative = "greater")
  
  out <- data.frame("Var1" = colnames(TRA.KLK.rawdata.clean[x[1]]),
                    "Var2" = colnames(TRA.KLK.rawdata.clean[x[2]]),
...
t.KLK1 = t.data[1:26, ]
t.KLK1
length(which(t.KLK1$p.value <= 0.05))
which(t.KLK1$p.value >= 0.05)
t.KLK1[13, ]
#The mean of KLK1 is significantly greater than all other means except from KLK1.3

t.KLK1.3 <- 

t.test(TRA.KLK.rawdata.clean$KLK1, TRA.KLK.rawdata.clean$KLK2, alternative = "greater")

length(which(cor.genes(TRA.KLK.breast.clean)==1))
```



# 3.2 Breast cancer GSE65216 (Maire et al. 2013)
...
samples.annotation.breast <- data.frame(matrix(ncol = 1, nrow = nrow(all.KLK.breast.clean.noGSM)))
colnames(samples.annotation.breast) <- "tumor type"                                 
rownames(samples.annotation.breast) <- rownames(all.KLK.breast.clean.noGSM)
ph.ind.Her2 = grep(pattern = "Her2", rownames(all.KLK.breast.clean.noGSM))
ph.ind.LumA = grep(pattern = "LumA", rownames(all.KLK.breast.clean.noGSM))
ph.ind.LumB = grep(pattern = "LumB", rownames(all.KLK.breast.clean.noGSM))
ph.ind.TNBC = grep(pattern = "TNBC", rownames(all.KLK.breast.clean.noGSM))
...
abline(h=mean(unlist(all.KLK.breast.clean)), col= "gray", lwd = 1)
dev.off()
...
## 3.3 Lung cancer GSE149507 (Cai et al. 2021)
...
rownames(all.KLK.lung.clean.noGSM)[1:12] <- sub("...........", "", rownames(all.KLK.lung.clean)[1:12])

# annotation for the samples
samples.annotation.lung <- data.frame(matrix(ncol = 1, nrow = nrow(all.KLK.lung.clean.noGSM)))
colnames(samples.annotation.lung) <- "sample"                                 
rownames(samples.annotation.lung) <- rownames(all.KLK.lung.clean.noGSM)
ph.ind.normal = grep(pattern = "n", rownames(all.KLK.lung.clean.noGSM))
ph.ind.cancer = grep(pattern = "ca", rownames(all.KLK.lung.clean.noGSM))
...
pheatmap(t(all.KLK.lung.clean.noGSM),
...
abline(v = median(lungExprs), col= "black", lwd = 2)#median all genes
abline(v = median(unlist(all.KLK.lung.clean)), col= "blue", lwd = 2) #median all KLKs
abline(v = median(unlist(TRA.KLK.lung.clean)), col= "red", lwd = 2) #median TRA KLKs
dev.off()
```
...
abline(h=mean(unlist(all.KLK.lung.clean)), col= "gray", lwd = 1)
dev.off()
...
wssplot <- function(rawdataExprs, nc=15, seed=1234)
{
  wss <- (nrow(rawdataExprs)-1)*sum(apply(rawdataExprs,2,var))
  for(i in 2:nc){
    set.seed(seed)
...
# Ideen später: 
#- einzelne PLots benennen und als Grid anordnen -> direkter Vergleich
#- bei Klk plots einzelne gene anzeigen lassen, auch als grid (?)
