...
# normally the first principal component are sufficient
...
# we can see in this cumulative variance plot, that we only need about 3 principal components to achieve a a total variance of 93,20 %
...
abline(h=median(unlist(all.KLK.breast.clean)), col= "blue", lwd = 2, lty = 2)
legend("topright", c("median all KLKs"), col=c("blue"), lwd=10)
dev.off()
...
###Hypothesis testing 
In the Boxplot and the heatmap KLK8.8 and 4.4 seem overexpressed. In the k-means both are in cluster 1.
```{r Hypothesis testing - Cluster 1}
...
#prepare datat for boxplot
#KLK8.8.sig <- b.KLK8.8vsall.sig[ ,1]
#KLK8.8non <- b.KLK8.8vsall.sig[ ,2]
#KLK4.4.sig <- b.KLK4.4.vsall.sig[ ,1]
#KLK4.4.non <- b.KLK4.4.vsall.sig[ ,2] 

#barplot(KLK8.8.sig, KLK8.8non, KLK4.4.sig, KLK4.4.non)

#results.cluster1.breast <- matrix(c("KLK8.8",71,1,"KLK4.4" ,72,0), nrow= 2, ncol = 3, byrow=TRUE)
#the non-significance from the genes vs. themself was subtracted 
#colnames(results.cluster1.breast) <- c("names", "significant", "non-significant")
#results.cluster1.breast

#ggplot(data=results.cluster1.breast, aes(x=names  ))+geom_bar(stat="identity")
  

#barplot(results.cluster1.breast, beside = TRUE)
#ggplot(results.cluster1.breast)



```
KLK8.8 and KLK4.4 are significantly over-expressed, compared to the other genes. Especially KLK8.8 is interesting because it is not an TRA. This means there is likely no central tolerance against this KLK, which makes it a good drug target. 


```{r, Hypothesis testing - cluster 4}
...
```
Conclusion: the KLKs in cluster four are not clearly higher expressed than the other genes. They only got in the same cluster together because the genes of cluster two are higher expressed in 5 of the chips (Tum01+0.4+06_TNBC, Tum103_LumB, Tum075_LumA). 


The main characteristic of this dataset is the subdivision into the different mutations. In the heatmap there are four genes, which are overexpressed in at least two of the five chips for one mutation. KLK10.3+KLK6 for Her2, KLK11+KLK11.2 for LumA. These foru genes are also found in the loadings of the PCA. In the following all KLKs, which are presented as the 12 main loadings. 
```{r Hypothesis testing - Subsets}
...
tl.breast.all
```
```{r Hypothesis testing - plot loadings}
...
#custom order 
tl.breast.loadings <- within(tl.breast.loadings, name <- factor(name, levels = c("KLK4.1", "KLK5", "KLK5.3", "KLK6", "KLK7", "KLK8.4", "KLK11", "KLK8.5","KLK10",  "KLK10.3", "KLK11.2", "KLK13.1")))
...
```
Some genes seem to be significantly higher expressed - proof this with the w-test
...
loadings.test <- tl.breast.loadings %>% 
  group_by(name) %>%
...
loadings.test
#Significant: 
#KLK4.1 LumA greater than TNBC
#KLK5: LumA greater than LumB
#KLK5.3 LumA greater than LumB
#KLK6: Her2 greater than LumB
#KLK13.1: LumA greater than TNBC
...
ggplot(tl.breast.loadings, aes(x=name, y=value))+
  ggtitle("PCA Loadings, GSE65216 (Maire et al. 2013) ")+
  geom_boxplot(aes(x=name, y=value, fill=mutation_type))+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
        )+
  scale_fill_manual("Mutation type", values =c("Her2"= "#FFFF00", "LumA"="#0000FF","LumB"= "#FF00FF","TNBC"="#00FF00"))+
  xlab("Genes") +
  ylab("Expression")+
  facet_wrap(~name, scales = "free_x")
  
  #stat_pvalue_manual(loadings.test, hide.ns = TRUE)
  
  #stat_compare_means(comparisons = combn(tl.breast.loadings$mutation_type, 2, simplify = FALSE), method="wilcox.test", paired=FALSE)
  #stat_pvalue_manual(loadings.test, hide.ns = TRUE, tip.length = 0, step.increase = 0) 
  #stat_compare_means(aes(group = mutation_type), label = "p.format", paired = FALSE)
...
```{r}
setwd(here("Plots/Plots_breast"))
pdf("Wraped_boxplotloadings_breast.pdf")
ggboxplot(tl.breast.loadings, x="name", y="value", facet.by="name", color = "mutation_type", scales = "free_x")+
  ggtitle("PCA Loadings, GSE65216 (Maire et al. 2013) ")+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())+
  xlab("Genes") +
...
#apparently optimal Number of clusters is 3


#visualization
results.km <- kmeans(scale(rawdataExprs.nodupl),3, nstart=25)
results.km$cluster
...
#all.KLK:rawdata.clean


wssplot <- function(all.KLK.rawdata.clean, nc=15, seed=1234)
{
  wss <- (nrow(all.KLK.rawdata.clean)-1)*sum(apply(all.KLK.rawdata.clean ,2,var))
  for(i in 2:nc){
    set.seed(seed)
    wss[i] <- sum(kmeans(all.KLK.rawdata.clean , centers = i)$withinss)}
  plot(1:nc, wss, type="b", xlab="Number of Clusters", ylab="Within groups sum of squares")
}

wssplot(all.KLK.rawdata.clean)



#approx 4 clusters

km.klk.rawdata <- kmeans(scale(all.KLK.rawdata.clean),5, nstart=25)
km.klk.rawdata$cluster

fviz_cluster(km.klk.rawdata, data=all.KLK.rawdata.clean, 
             palette =c("#FF66CC","#00AFBB", "#FC4E07", "#E7B800", "#8DD3C7"), 
             geom = c("point","text") ,
             ellipse.type = "convex",
             ggtheme = theme_bw(),
             labelsize = 6,
...
##labelsize und text bei restlichen noch, repel anpassen

#km.klk.lung.7 <- kmeans(scale(all.KLK.lung.clean.noGSM),7, nstart=25)
#km.klk.lung.7$cluster

#fviz_cluster(km.klk.lung.7, data=all.KLK.lung.clean.noGSM, 
            # palette =c("#2E9FDF","#00AFBB", "#FC4E07", "#E7B800", "#009900", "#3399FF", "#003333"), 
            # geom = "point", 
            # ellipse.type = "convex",
            # ggtheme = theme_bw(),
...
#i would choose the 5 cluster plot, since clusters are more distinct there
...
#by hand 
#KLK11
results[1,3]<- wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK11", ]), as.matrix(all.KLK.lung.clean.n["KLK11", ]), paired = TRUE)$p.value
#KLK11.2
results[2,3]<-wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK11.2", ]), as.matrix(all.KLK.lung.clean.n["KLK11.2", ]), paired = TRUE)$p.value
#KLK12
results[3,3]<-wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK12", ]), as.matrix(all.KLK.lung.clean.n["KLK12", ]), paired = TRUE)$p.value
#KLK12.3
results[4,3]<-wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK12.3", ]), as.matrix(all.KLK.lung.clean.n["KLK12.3", ]), paired = TRUE)$p.value
#KLK12.4
results[5,3]<-wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK12.4", ]), as.matrix(all.KLK.lung.clean.n["KLK12.4", ]), paired = TRUE)$p.value
#KLK12.5
results[6,3]<-wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK12.5", ]), as.matrix(all.KLK.lung.clean.n["KLK12.5", ]), paired = TRUE)$p.value
#KLK12.6
results[7,3]<-wilcox.test(as.matrix(all.KLK.lung.clean.ca["KLK12.6", ]), as.matrix(all.KLK.lung.clean.n["KLK12.6", ]), paired = TRUE)$p.value
results

identical(as.matrix(all.KLK.lung.clean.ca["KLK12.3", ]), as.matrix(all.KLK.lung.clean.ca["KLK12.4", ]))
#KLK12-12.5 have the same p.value of 0.0625, although their values are not identical, they are so similar that they have the same p-value. 
...
```{r, Hypothesis testing Cluster 1}
#Hypothesis testing cluster 1

#data needs the format of three columns (name, value, tissue type)
...
#subset dataframe: only genes of cluster1
cluster1.lung.names <- c("KLK12", "KLK12.1", "KLK12.3", "KLK12.4", "KLK12.5")
cluster1.lung.names <- as.matrix(cluster1.lung.names)
b.cluster5.lung <- tl.all.KLK.lung.clean[, 1] %in% cluster3.lung.names
tl.cluster5.lung <- which(b.cluster5.lung == TRUE)
tl.cluster5.lung <- as.data.frame(tl.all.KLK.lung.clean[tl.cluster5.lung, ])
tl.cluster5.lung

#Plot with p.values cluster 5 
setwd(here("Plots/Plots_lung"))
pdf("Cluster1_lung.pdf")
ggplot(tl.cluster5.lung, aes(x= name, y=value, fill= tissue_type))+
  ggtitle("Cluster 1, GSE149507 (Cai et al. 2021)")+
  stat_boxplot(geom="errorbar")+
  geom_boxplot()+
  stat_compare_means(aes(group = tissue_type), label = "p.format", paired = TRUE, size = 3.5)
dev.off()

#check p.value KLK1.3
wilcox.test(t.all.KLK.lung.clean.ca$KLK1.3, t.all.KLK.lung.clean.n$KLK1.3, paired = TRUE)
```


```{r, Cluster 2}
#Check Cluster 2 from the kmeans
#filter the KLKs from tl.lung df
cluster2.lung.names <- c("KLK4.4",  "KLK8.8", "KLK10.3", "KLK11", "KLK11.2")
b.cluster2.lung <- tl.all.KLK.lung.clean[, 1] %in% cluster4.lung.names
tl.cluster2.lung <- which(b.cluster4.lung == TRUE)
tl.cluster2.lung <- as.data.frame(tl.all.KLK.lung.clean[tl.cluster2.lung, ])
tl.cluster2.lung

#ggplot 
setwd(here("Plots/Plots_lung"))
pdf("Cluster2_lung.pdf")
ggplot(tl.cluster2.lung, aes(x= name, y=value, fill= tissue_type))+
  scale_fill_manual(labels=c("cancerous", "normal"),
                    values= c("red", "blue"))+
  ggtitle("Cluster 2, GSE149507 (Cai et al. 2021)")+
  stat_boxplot(geom="errorbar")+
  geom_boxplot()+
  stat_compare_means(aes(group = tissue_type), label = "p.format", paired = TRUE, size = 3.5)
dev.off()
...
```{r, PCA genes}
PCA.lung.names <- c("KLK1.3", "KLK4.4", "KLK6", "KLK12", "KLK13.1", "KLKB1.1", "KLKB1.3")
b.PCA.lung <- tl.all.KLK.lung.clean[, 1] %in% PCA.lung.names
tl.PCA.lung <- which(b.PCA.lung == TRUE)
tl.PCA.lung <- as.data.frame(tl.all.KLK.lung.clean[tl.PCA.lung, ])
tl.PCA.lung

setwd(here("Plots/Plots_lung"))
pdf("PCA_Hyp_lung.pdf")
ggplot(tl.PCA.lung, aes(x= name, y=value, fill= tissue_type))+
  ggtitle("PCA Genes, GSE149507 (Cai et al. 2021)")+
  stat_boxplot(geom="errorbar")+
  geom_boxplot()+
  stat_compare_means(aes(group = tissue_type), label = "p.format", paired = TRUE, size = 3.5)
dev.off()
...
#Conditions for using regression analysis:
#the residuals are normally distributed 
#no correlation between the residuals and the explanatory variable 
reg.tissue.type.all <- lm(tissue_type ~ KLK12+KLK10.3+KLK12+KLK12.3+KLK12.4+KLK12.5+KLK12.6, data = reg.lung.all)
summary(reg.tissue.type)


reg.tissue.type <- lm(tissue_type ~ KLK10.3, data = reg.lung.all)
summary(reg.tissue.type)

hist(reg.tissue.type$residuals, breaks=10)
qqnorm(reg.tissue.type$residuals);qqline(reg.tissue.type$residuals)


cor(reg.lung.all$KLK10.3, reg.tissue.type$residuals)
plot(reg.lung.all$KLK10.3, reg.tissue.type$residuals, pch=20)

plot(reg.lung.all$tissue_type,reg.tissue.type$fitted.values,pch=20,col='blue', xlab='Real values',ylab='Predicted values');abline(0,1,col='red')


```

```{r, logistic regression Model 0}
...
#Model 0 with all genes 
model0 <- glm(as.factor(tissue_type)~ ., family = binomial(), data = reg.lung.all)
summary(model0)
#error 61 not defined because of singularities??? -> Collinearity! 
#eliminate all genes with a correlation higher than 0.7

tmp <- cor(reg.lung.all)
tmp[upper.tri(tmp)]<- 0
diag(tmp) <- 0
length(which(tmp > 0.7))
#with 0.7 still Colliniarity - set to 0.6
low.cor.reg.lung.all <- reg.lung.all[ , !apply(tmp, 2, function(x){ any(x>0.6)})]
low.cor.reg.lung.all
#only 10 genes left!
#glm with the low correlation dataset
model0.1<- glm(as.factor(tissue_type)~ ., family = binomial(), data = low.cor.reg.lung.all)
sum.model0.1 <- summary(model0.1)
sum.model0.1
#the for every gene Std.Error is enormous and the p.value is always 1! Something is wrong!
#we probably a separation in our dataset
#1. Fisher scoring iterations is 23 (4-5 is typical)
#2. huge standard errors
#Rule from literature: 10 outcome events per predictor variable! 
ggplot(sum.model0.1, aes(GSM4504101_SCLC_01_ca, GSM4504103_SCLC_02_ca   ))
plot(model0.1)


sep.model0.1<- glm(tissue_type~ .,data = low.cor.reg.lung.all, family = binomial("logit"), method = "detect_separation")
sep.model0.1
#there is separation in the dataset! Infinite estimates for coefficients


#predict Model 0.1
pred.model0.1 <- predict(model0.1, test.reg, type="response")
pred.model0.1
actual0.1 <- c(1, 0, 0, 0)
separationplot(pred.model0.1, actual0.1, type = "line", line = T)
...
```{r, Logistic Regression Model 1+2}


#Model 1: all genes which have the highest p.value when put into individually into the function.  
#Identifying the genes 
reg.model <- apply(minus.dat, 2, function(x){
  sum <- summary(glm(as.factor(tissue_type)~x, family = binomial(), data = reg.lung.all))
  out <- sum
  })
reg.model
reg.sig.05 <- which(reg.model[2, ] < 0.05) #only KLKB1 is significant 
reg.sig.05#one gene not enough -> set level to 0.1
reg.sig.10 <- which(reg.model[2, ] < 0.1)
reg.sig.10 #KLK1, KLK5.3, KLK7, KLKB1, KLKB1.1, KLKB1.3

model1 <- glm(tissue_type ~ KLK1+KLK5.3+KLK7+KLKB1+KLKB1.1+KLKB1.3, data = train.reg, family = binomial())
model1.sum <- summary(model1)
model1.sum #why is the z value=0 and the p.value=1???? something is wrong!

#Modell 2: genes from cluster 3,4,5 which the lowest p.value 
#KLK10.3, KLK1.3, KLK12 (KLK12.1 ect. noz incorporated due to Colinearity), KLK4.4
model2 <- glm(tissue_type~KLK1.3+KLK10.3+KLK12+KLK4.4, family = binomial(), data = reg.lung.all)
...
pred.model1 <- predict(model1, test.reg, type="response")
as.data.frame(pred.model1)
#imprecise 2/4 were false

#prediction with model 2
pre.model2 <- predict(model2, test.reg, type = "response")
as.data.frame(pre.model2)
...
```{r}
#neural network 1, incorporate all genes 
nn1 <- neuralnet(tissue_type ~., data=train.reg, hidden = c(10,10,5), act.fct = "logistic")
nn1$result.matrix
plot(nn1)

nn1.results <- compute(nn1, test.reg)
pred.nn1.results <- data.frame(actual = test.reg$tissue_type, prediction = nn.results$net.result)
pred.nn1.results #why two predictions? 
#the model can differentiate very accurately between the different chip types!
#further quality test needed! first solve problem of two predictions!

#with the most significant genes of the clusters
nn2 <- neuralnet(as.factor(tissue_type) ~ KLK1.3+KLK10.3+KLK12+KLK4.4, data=train.reg, hidden = c(10,10,5), act.fct = "logistic")
nn2$result.matrix
plot(nn2)
nn2.results <- compute(nn2, test.reg)
pred.nn2.results <- data.frame(actual = test.reg$tissue_type, prediction = nn.results$net.result)
pred.nn2.results
