...
for (j in rownames(all.KLK.lung.clean)[cluster3.lung.names]) {
  p[[j]] <- ggplot(data=all.KLK.lung.clean.ca, aes_string(x=grepl("ca", colnames(all.KLK.lung.clean)), y=j)) + # Specify dataset, input or grouping col name and Y
            geom_boxplot(aes(fill=factor(grepl("ca", colnames(all.KLK.lung.clean))))) + guides(fill=FALSE) + # Boxplot by which factor + color guide
