...
dim(TRA.KLK.rawdata) ## 39 KLK genes in breast cancer rawdata 
...
length(TRA.KLK.breast[1,] == TRA.KLK.breast[10,] )
...
We noticed that there are two exact identical KLK rows, which should get removed.  
```{r Short clean up}
########## !!!! Fast alle KLK gene die TRAs sind haben identische Expressions-werte. Das kann eig. nicht sein, wenn das mit 8 nachkommastellen alle werte identisch sind. Das sind immer die KLK1 und KLK1.1 und 1.2 usw. 
## Entweder identische Zeilen löschen oder habt ihr einen Plan was man damit machen soll?


# We noticed that there are two exact identical KLK rows, 
ind.human[1]
ind.human[10]
...
# For first breast cancer data set
TRA.KLK.rawdata = rawdataExprs[ind.KLK.tra.human,]
TRA.KLK.rawdata = TRA.KLK.rawdata[-10,]
dim(TRA.KLK.rawdata) # 38 10

setwd(here("Plots/Plots_raw_breast"))
pdf("KLK_TRAs_Heatmap_rawdata.pdf")
pheatmap(TRA.KLK.rawdata, main = "Expression values of KLK TRAs in human breast cancer GSE27830 ")
## (Nagel et al. 2012)
dev.off()
...
TRA.KLK.breast = TRA.KLK.breast[-10,]
dim(TRA.KLK.breast) ## 38 KLK genes in breast cancer rawdata 

setwd(here("Plots/Plots_breast"))
pdf("KLK_TRAs_Heatmap_breast.pdf")
pheatmap(TRA.KLK.breast, main = "Expression values of KLK TRAs in human breast cancer GSE65216")
## (Maire et al. 2013)
dev.off()
...
TRA.KLK.lung = TRA.KLK.lung[-10,]
dim(TRA.KLK.lung) ## 38 KLK genes in breast cancer rawdata 

setwd(here("Plots/Plots_lung"))
pdf("KLK_TRAs_Heatmap_lung.pdf")
pheatmap(TRA.KLK.lung, main = "Expression values of KLK TRAs in human lung cancer GSE149507")
## (Cai et al. 2021)
dev.off()
...
### MATRIX TRA.all.KLK -> all KLK genes in breast cancer
...
boxplot(all.KLK.rawdata,col=rainbow(length(rownames(all.KLK.rawdata))),main="gene expression of KLK genes in human breast cancer",cex.axis=0.8)
...
Test for correlation between KLKs rawdata breast
```{r}
#test turning all.KLK.rawdata into dataframe
KLK.dat.rawdata <- data.frame(all.KLK.rawdata)
dim(KLK.dat.rawdata)
#calculate correlation
cor.rawdata.breast <- cor(all.KLK.rawdata, method = "pearson")
round(cor.rawdata.breast, 2)
cor.rawdata.breast <- data.frame(cor.rawdata.breast)
#delete diagonal 
diag(cor.rawdata.breast)=0
cor.rawdata.breast[upper.tri(cor.rawdata.breast)]= 0

cor.rawdata.breast
length(which(cor.rawdata.breast == 1))
#There are identical columns! corr=1

#Example KLK7, KLK7.1
plot(KLK.dat.rawdata$KLK7, KLK.dat.rawdata$KLK7.1,
     pch=20,
     xlab = "KLK7 gene expression",
     ylab = "KLK7.1 gene expression")
abline(0,1, col="gray")

cor(KLK.dat.rawdata$KLK7, KLK.dat.rawdata$KLK7.1)

#Remove identical columns
KLK.rawdata.removed <- KLK.dat.rawdata[!duplicated(unclass(KLK.dat.rawdata))]
dim(KLK.rawdata.removed)
KLK.rawdata.removed

#check for identical 
cor.KLK.rawdata.removed <- data.frame(cor(KLK.rawdata.removed))
diag(cor.KLK.rawdata.removed)=0
cor.KLK.rawdata.removed[upper.tri(cor.KLK.rawdata.removed)]= 0
length(which(KLK.rawdata.removed == 1))
